namespace Fizzler
{
    ...
    public static class Tokener
    {
        ...
        private static Token ParseString(Reader reader, char quote)
        {
            ...
            while ((ch = reader.Read()) != quote)
            {
                ...
                if (ch == '\\')
                {
                    ...
                    {
                        var n = (ParseHex(reader.Read()) << 4) +
                                (ParseHex(reader.Read()) << 0);
                        sb.Append((char)n);
                    }
                    else if (ch == 'u')
                    {
                        var n = (ParseHex(reader.Read()) << 12) +
                                (ParseHex(reader.Read()) << 8) +
                                (ParseHex(reader.Read()) << 4) +
                                (ParseHex(reader.Read()) << 0);
                        sb.Append((char)n);
                    }
                    else
        ...
        private static int ParseHex(char v)
        {
            if (v >= '0' && v <= '9') return v - '0';
            else if (v >= 'a' && v <= 'f') return v - 'a' + 10;
            else if (v >= 'A' && v <= 'F') return v - 'A' + 10;
            else throw new ArgumentException("Invalid hex character: " + v.ToString());
        }
        ...
        private sealed class Reader
        {
            ...
            public char Value { get { return Ready ? _input[_index] : (char)0; } }
            ...
            internal string Marked(int trim)
            {
                var start = _start;
